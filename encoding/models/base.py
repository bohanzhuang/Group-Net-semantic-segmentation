...
from . import resnet, resnet_binary, resnet_binary_dilated
...
class BaseNet(nn.Module):
    def __init__(self, nclass, backbone, aux, se_loss, dilated=True, norm_layer=None,
        ...
        if backbone == 'resnet18':
            ...
            self.pretrained = resnet.resnet152(pretrained=True, dilated=dilated,
                                               norm_layer=norm_layer, root=root)
        elif backbone == 'binary_resnet18':
            self.pretrained = resnet_binary.resnet18(pretrained=True, dilated=dilated,
                                              norm_layer=norm_layer, root=root)   
        elif backbone == 'binary_resnet34':
            self.pretrained = resnet_binary.resnet34(pretrained=True, dilated=dilated,
                                              norm_layer=norm_layer, root=root)
        elif backbone == 'binary_resnet50':
            self.pretrained = resnet_binary.resnet50(pretrained=True, dilated=dilated,
                                              norm_layer=norm_layer, root=root)
        elif backbone == 'binary_resnet18_dilated':
            self.pretrained = resnet_binary_dilated.resnet18(pretrained=True, dilated=dilated,
                                              norm_layer=norm_layer, root=root)   
        elif backbone == 'binary_resnet34_dilated':
            self.pretrained = resnet_binary_dilated.resnet34(pretrained=True, dilated=dilated,
                                              norm_layer=norm_layer, root=root)
        elif backbone == 'binary_resnet50_dilated':
            self.pretrained = resnet_binary_dilated.resnet50(pretrained=True, dilated=dilated,
                                              norm_layer=norm_layer, root=root)
    ...
    def base_forward(self, x):
        if self.backbone.startswith('wideresnet'):
            ...
            sep_out = None
            sum_out = x
            for layer in self.pretrained.layer1: 
                sep_out, sum_out = layer(sep_out, sum_out)
            c1 = sum_out
            for layer in self.pretrained.layer2:
                sep_out, sum_out = layer(sep_out, sum_out)
            c2 = sum_out
            for layer in self.pretrained.layer3:
                sep_out, sum_out = layer(sep_out, sum_out)
            c3 = sum_out
            for layer in self.pretrained.layer4:
                sep_out, sum_out = layer(sep_out, sum_out)
            c4 = sum_out
        return c1, c2, c3, c4
